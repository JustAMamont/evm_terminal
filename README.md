...
> **Note:**
> Так как вся бизнес-логика находится и работает на устройстве пользователя, важно дожидаться подтверждения транзакции перед отправкой следующей (оповещение об "успешности покупки/продажи", отображается в интерфейсе) иначе могут быть ошибки (reverted).
