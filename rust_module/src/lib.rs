...
fn dexbot_core(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(bridge::pop_event_from_ring, m)?)?;
    m.add_function(wrap_pyfunction!(bridge::push_command_to_ring, m)?)?;
    m.add_function(wrap_pyfunction!(bridge::ring_available, m)?)?;
    m.add_function(wrap_pyfunction!(bridge::process_commands, m)?)?;
